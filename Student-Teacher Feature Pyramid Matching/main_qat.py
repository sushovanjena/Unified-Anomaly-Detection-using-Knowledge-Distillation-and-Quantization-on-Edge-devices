...
def main():
    ...
    if args.split == 'train':
        image_list = sorted(glob(os.path.join(args.mvtec_ad, args.category, 'img', 'train_good*.png')))
        print('image_list')
...
def train_val(teacher, student, train_loader, val_loader, args):
    ...
    backend = 'fbgemm'
    teacher.qconfig = torch.ao.quantization.get_default_qat_qconfig(backend)
    student.qconfig = torch.ao.quantization.get_default_qat_qconfig(backend)
    torch.backends.quantized.engine = backend

    qteacher = torch.ao.quantization.prepare_qat(teacher,inplace = True)
    qstudent = torch.ao.quantization.prepare_qat(student,inplace = True)

    min_err = 10000
    qteacher.eval()
    qstudent.train()

    optimizer = torch.optim.SGD(qstudent.parameters(), 0.4, momentum=0.9, weight_decay=1e-4)
    for epoch in range(args.epochs):
        ...
        for batch_data in train_loader:
            ...
            with torch.no_grad():
                t_feat = qteacher(batch_img)
            s_feat = qstudent(batch_img)
        ...
        err = test(qteacher, qstudent, val_loader).mean()
        print('Valid Loss: {:.7f}'.format(err.item()))

        print("Converting to QAT Model ")
        qstudent.to('cpu')
        torch.quantization.convert(qstudent, inplace=True)
        print("Quantized model:", qstudent)


        qteacher.to('cpu')
        torch.quantization.convert(qteacher, inplace=True)
        print("Teacher Model Converted Successfully ")
